...
export default {
  ...
  watch: {
    model(newValue) {
      if (newValue == true) {
        this.$store.commit('folderbrand/setClearForm')
    ...
    isErrors(newValue) {
      if (!newValue) {
        this.$store.commit('folderbrand/setIsErrors', '')
      }
    }
  },
  methods: {
    ...mapActions('folderbrand', ['storeData']),
    ...mapMutations('folderbrand', ['setLoading']),
    submit() {
  ...
  computed: {
    ...mapState('folderbrand', {
      errors: state => state.errors,
      isErrors: state => state.isErrors
    }),
    model: {
      get() {
        return this.$store.state.folderbrand.model
      },
      set(model) {
        this.$store.commit('folderbrand/setModel', model)
      }
    },
    loading: {
      get() {
        return this.$store.state.folderbrand.loading
      },
      set(loading) {
        this.$store.commit('folderbrand/setloading', loading)
      }
    }
  }
}
