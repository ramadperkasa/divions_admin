                      ...
                      <v-col cols="2" v-for="(item, index) in lists.data" :key="index">
                        <div class="container-img">
                          ...
                          <template v-else>
                            <img
                              :src="
                                 item.type == 0 ?  large(process.env.API_URL +item.image) : item.type == 2 ? 'https://img.youtube.com/vi/' +item.image+'/0.jpg' : 'https://img.youtube.com/vi/' +item.image+'/0.jpg' 
                              "
                              class="hover--image"
                              @click="$refs.check[index].$refs.input.click()"
                              alt
                    ...
                    <template v-for="(item,index) in listNews ? listNews.data : ''">
                      <v-col v-if="!multiple" :key="index" cols="3">
                        <v-card>
                          ...
                          <v-img v-else :src="process.env.API_URL+item.cover_image" height="200px"></v-img>
...
export default {
  ...
  data() {
    return {
      ...
      url: '',
      dropzoneOptions: {
        url: process.env.API_URL + 'api/web/brand-image/plural/'+this.$store.state.app.app,
        thumbnailWidth: 150,
  ...
  computed: {
    ...mapState({
      ...
      lists: state => state.brandupload.lists,
      total: state => state.brandupload.total,
      model: state => state.brandupload.model,
      listNews: state => state.brandupload.news,
      total_news: state => state.brandupload.total_news
    }),
    model: {
      get() {
      ...
      set(model) {
        this.$store.commit('brandupload/setModel', model)
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    fetchNews() {
      ...
      return new Promise((resolve, reject) => {
        var result = this.$store.dispatch('brandupload/getNews', {
    ...
    fetch() {
      ...
      if (this.imageOnly == '' || this.imageOnly == true) {
        return new Promise((resolve, reject) => {
          var param  = {...this.params, brand_id : this.$store.state.app.app}
          var result = this.$store.dispatch('brandupload/getDataImage', {
            params: param
          })
        ...
        return new Promise((resolve, reject) => {
          var param  = {...this.params, brand_id : this.$store.state.app.app}
          var result = this.$store.dispatch('brandupload/getData', {
            params: param
          })
          result.finally(() => {
            this.loading = false
    ...
    dataUpload() {
      return new Promise((resolve, reject) => {
        var result = this.$store.dispatch('brandupload/postDataUpload', {
          embed: this.embed
        })
        result.finally(() => {
          this.closeDialog()
