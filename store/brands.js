export const state = () => ({
  ...
  export const mutations = {
    ...
    setDialogDestroys(state, payload) {
      if (payload == false) {
        state.item = ''
        state.showDialogDestroys = payload
      } else {
  ...
  export const actions = {
    getPermission({ commit, state, rootState }, payload) {
      return new Promise((resolve, reject) => {
        if (
          rootState.auth.user.all_permissions.includes('create-' + payload) &&
          rootState.auth.user.all_permissions.includes('delete-' + payload)
        ) {
    ...
    getData({ commit, state, rootState }, search) {
      commit('setListData', [])
        ...
        this.$axios
          .get('/api/web/' + state.url, {
            params: {
              ...
              parent_id: state.options.parent_id ? state.options.parent_id : '',
              brand_id : rootState.app ? rootState.app.app : ''
            }
          })
          .then(response => {
    ...
    storeData({ dispatch, commit, state, rootState }, payload) {
      return new Promise((resolve, reject) => {
        this.$axios
          .post('/api/web/' + state.url + '/update-or-create', {payload:payload, brand_id:brand_id})
          .then(response => {
    ...
    reorder({ dispatch, commit, state }, item) {
      return new Promise((resolve, reject) => {
        this.$axios
          .post('/api/web/' + state.url + '/reorder', item)
          .then(response => {
            commit('setSnackbar', true)
            if (response.status) {
