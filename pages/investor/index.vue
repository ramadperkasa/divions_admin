    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page="Investor /"
            current_page="List Inverstor"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-incognito</v-icon>
          <v-toolbar-title>Daftar Investor</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
          ...
          <v-row>
            ...
            <v-col cols="8">
              <v-text-field
                ...
                :error="errors.investor_name ? true : false"
                :error-messages="
                  errors.investor_name ? errors.investor_name[0] : ''
                "
                outlined
                dense
              ></v-text-field>
...
import datatable from '@/datatable/investor/index.json'
...
export default {
  ...
  head() {
    return {
      title: 'Investor: List Investor'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'investor')])
  ...
  watch: {
    ...
    isClickEdit(newValue) {
      if (newValue == true) {
        ...
        this.form.cover_image = this.itemEdit.cover_image
        this.isEdit = true
        this.btn_new = false
      }
    },
