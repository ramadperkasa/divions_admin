        ...
        <v-card class="custom-login-form" elevation="2" :disabled="isLoading">
          ...
          <v-container class="pa-10">
            <v-row align="center" justify="center" no-gutters>
              ...
              <v-col cols="12" class="pb-5" v-if="isbrand === 'true'">
                <v-btn class="btn-primary" @click="openLoginBrand()" block
                  >Masuk Ke Brand</v-btn
                >
              </v-col>
              <!-- <v-col cols="12" class="pb-5">
                <v-btn
...
export default {
  ...
  methods: {
    ...
    openLoginBrand() {
      window.open(process.env.BRAND_URL, '_blank')
    },
    async facebookLogin() {
      const newWindow = openWindow('', 'message')
      const url = await this.$axios
        .post('http://localhost:8000/api/oauth/facebook')
        .then(response => {
    ...
    onMessage(e) {
      if (e.data.access_token) {
        this.$auth.setToken('local', e.data.access_token)
