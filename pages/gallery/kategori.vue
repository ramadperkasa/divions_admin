    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Gallery /"
            current_page="Kategori Gallery"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-image-album</v-icon>
          <v-toolbar-title>Daftar Kategori Gallery</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
...
import datatable from '@/datatable/gallery/kategori.json'
...
export default {
  ...
  head() {
    return {
      title: 'Gallery: Kategori Gallery'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'kategori-gallery')])
