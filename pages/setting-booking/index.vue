  ...
  <v-layout justify-center align-center>
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Setting Booking /"
            current_page="Daftar Setting Booking"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-home</v-icon>
          <v-toolbar-title>Daftar Setting Booking</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
...
import datatable from '@/datatable/setting-booking/index.json'
...
export default {
  ...
  head() {
    return {
      title: 'Setting Booking: Daftar Setting Booking'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'setting-booking')])
