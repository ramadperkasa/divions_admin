    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Berita /"
            current_page="Kategori Berita"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-newspaper</v-icon>
          <v-toolbar-title>Daftar Kategori Berita</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
...
import datatable from '@/datatable/berita/kategori.json'
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'kategori-berita')])
