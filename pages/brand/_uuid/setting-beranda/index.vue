        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:item.action="{ item }">
              <action :item="item" edit destroy viewContent="content"></action>
            </template>
          </v-data-table>
        </v-card-text>
      </v-card>
    </v-flex>
    <v-dialog v-model="dialog.form.model" max-width="80%" persistent scrollable>
      <v-card>
...
import datatable from '@/datatable/brands/setting-beranda/index.json'
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'brand-setting-beranda')])
