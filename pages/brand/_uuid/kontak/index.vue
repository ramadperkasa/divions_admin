    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        ...
        <v-toolbar dense flat>
          ...
          <v-toolbar-title>Daftar Kontak Brand</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
    ...
    >
      <v-card>
        ...
        <v-card-text>
          ...
          ></v-text-field>
          <v-text-field
            label="Nama Kontak Brand*"
            hint="Username atau Nickname kontak Brand"
            v-model="form.isi"
          ...
          <v-row v-else>
            ...
            <small class="ml-5" @click="getKoor()" style="cursor:pointer"
...
import datatable from '@/datatable/brands/kontak/index.json'
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'brand-kontak')])
