    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Uploads /"
            current_page="Folder Product"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-folder-multiple-outline</v-icon>
          <v-toolbar-title>Daftar Folder Product</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
...
import datatable from '@/datatable/brands/uploads/folder.json'
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'brand-folder')])
