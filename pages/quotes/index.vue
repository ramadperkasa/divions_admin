<template>
  <v-layout justify-center align-center>
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb page=" Quotes /" current_page="List Quotes"></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-comment-quote</v-icon>
          <v-toolbar-title>Daftar Quotes</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:item.foto="{ item }">
              <table-image v-model="item.images" type="0"></table-image>
            </template>
    ...
    <v-dialog v-model="dialog.form.model" scrollable persistent max-width="900px">
      <v-card>
        ...
        <v-card-text>
          <v-row>
            <v-col cols="4">
              <select-image v-model="form.foto" :isEdit="$route.params.id"></select-image>
              <v-btn block text color="info" @click="dialog.image.model = true">
                <v-icon left>{{$route.params.id ? 'mdi-image-edit' : 'mdi-image-plus'}}</v-icon>
            ...
            <v-col cols="8">
              <v-text-field
                label="Oleh *"
                v-model="form.oleh"
                :error="errors.oleh ? true : false"
                :error-messages="errors.oleh ? errors.oleh[0] : ''"
                outlined
                dense
              ></v-text-field>
              <v-text-field
                ...
                :error-messages="errors.jabatan ? errors.jabatan[0] : ''"
                outlined
                dense
              ></v-text-field>
              <v-textarea
                ...
                :error-messages="errors.deskripsi ? errors.deskripsi[0] : ''"
                outlined
                dense
              ></v-textarea>
            </v-col>
...
import datatable from '@/datatable/quotes/index.json'
...
export default {
  ...
  head() {
    return {
      title: 'Quotes: List Quotes'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'quotes')])
  ...
  methods: {
    ...
    openImage(item) {
      this.src = process.env.API_URL + item.foto
      this.types = 0
      return new Promise((resolve, reject) => {
