    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Vacancy /"
            current_page="Kategori Vacancy"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-newspaper</v-icon>
          <v-toolbar-title>Daftar Kategori Vacancy</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
          ...
          >
            ...
            <template v-slot:item.action="{ item }">
              <action
                ...
                hide
                destroy
                :child="['sub_kategori_vacancy_count']"
              ></action>
            </template>
          </v-data-table>
        </v-card-text>
...
import datatable from '@/datatable/vacancy/kategori.json'
...
export default {
  ...
  head() {
    return {
      title: 'Vacancy: Kategori Vacancy'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'vacancy-kategori')])
