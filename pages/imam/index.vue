    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Imam /"
            current_page="Daftar Imam"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-account-tie-voice</v-icon>
          <v-toolbar-title>Daftar Imam</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:item.action="{ item }">
              <action :item="item" edit hide destroy></action>
            </template>
          </v-data-table>
        </v-card-text>
      </v-card>
...
import datatable from '@/datatable/imam/index.json'
...
export default {
  ...
  head() {
    return {
      title: 'Imam: Daftar Imam'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'imam')])
