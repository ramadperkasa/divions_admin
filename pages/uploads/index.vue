    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb
            page=" Uploads /"
            current_page="List Upload"
          ></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-upload-multiple-outline</v-icon>
          <v-toolbar-title>Daftar Upload</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
            <search-data
              v-model="search"
        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:item.image="{ item }">
              <table-image v-model="item.image" :type="item.type"></table-image>
            </template>
                    ...
                    <v-menu absolute offset-y style="max-width: 600px">
                      <template v-slot:activator="{ on }">
                        ...
                        <v-img
                          ...
                          :src="
                            item.type == 0
                              ? large(process.env.API_URL + item.image)
                              : item.type == 2
                              ...
                              : false
                          "
                          :lazy-src="xsmall(process.env.API_URL + item.image)"
                          aspect-ratio="4:3"
                          width="100%"
                          height="100%"
          ...
          <v-row>
            <v-col cols="4">
              ...
              <v-btn
                ...
                color="info"
              >
                <v-icon left>mdi-plus</v-icon>Tambah Media
              </v-btn>
              ...
              <input
                ...
                @change="fileSelected"
              />
              <small class="error--text" v-if="errors.image">
...
import datatable from '@/datatable/uploads/index.json'
import dialogFolder from '~/components/dialog-folder.vue'
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'image')])
    return
  },
  data() {
    return {
  ...
  methods: {
    ...
    setDialogFolderModel() {
      this.$store.commit('folder/setModel', true)
    },
    xsmall(val) {
