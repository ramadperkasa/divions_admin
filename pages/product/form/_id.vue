<template>
  <div>
    <v-card :loading="isLoadingPage" :disabled="isLoadingPage">
      <v-card-title>
        <v-icon left>mdi-pencil</v-icon>Produk
      </v-card-title>
      <v-divider></v-divider>
      <v-card-text>
        <v-row>
            ...
            <v-row>
              ...
              <v-col cols="2">
                <v-row>
                  ...
                  <v-tooltip top>
                    <template v-slot:activator="{ on }">
                      <v-btn v-on="on" icon @click="setDialogProdukKategoriModel()">
    ...
    <dialog-upload
      ...
      :media="true"
    ></dialog-upload>
    <dialog-brand-kategori @update="fetchProdukKategori()"></dialog-brand-kategori>
  </div>
</template>
...
export default {
  ...
  watch: {
    brands: {
      ...
      this.form.image = newValue.cover_image,
      this.form.brand_kategori_id = eval(newValue.brand_kategori_id)
      },
      deep: true
    },
