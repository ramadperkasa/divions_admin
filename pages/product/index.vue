    ...
    <v-flex xs12 sm12 md12>
      <v-card>
        ...
        <v-toolbar dense flat>
          ...
          ></destroy>
          <create @click.native="$router.push('/product/form')"></create>
          ...
          >
            ...
            <template v-slot:item.cover_image="{ item }">
              <video
                ...
                @click="openImage({cover_image: item.cover_image, type: 1})"
                class="grey lighten-2 ma-1 hover--image"
              >Browser kamu ga support untuk menjalankan video ini</video>
              <v-img
                @click="openImage({cover_image: item.cover_image, type: 2})"
                aspect-ratio="1"
                height="40"
                width="50"
                class="grey lighten-2 ma-1 hover--image"
                v-else-if="item.type_img == 2"
                :src="`https://img.youtube.com/vi/`+item.cover_image+`/0.jpg`"
              >
              ...
              <v-img
                v-else
                :src="process.env.API_URL + item.cover_image"
                :lazy-src="process.env.API_URL + item.cover_image"
                @click="openImage({cover_image : item.cover_image, type : 0})"
                aspect-ratio="1"
                height="40"
                width="50"
                class="grey lighten-2 ma-1 hover--image"
...
import datatable from '@/datatable/product/index.json'
