<template>
  <v-layout justify-center align-center>
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb page=" Subscribe /" current_page="List Subscribe"></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-account-details-outline</v-icon>
          <v-toolbar-title>Daftar Subscribe</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
    ...
    <v-dialog v-model="dialog.form.model" scrollable persistent max-width="700px">
      <v-card>
...
import datatable from '@/datatable/subscribe/index.json'
...
export default {
  ...
  head() {
    return {
      title: 'Gallery: Subscribe'
    }
  },

  async fetch({ store }) {
    await Promise.all([store.commit('setUrl', 'subscribe')])
