<template>
  <v-layout justify-center align-center>
    <v-flex xs12 sm12 md12>
      <v-card>
        <v-col cols="12">
          <alert-breadcrumb current_page="Produk"></alert-breadcrumb>
        </v-col>
        <v-toolbar dense flat>
          <v-icon class="mr-2">mdi-office-building</v-icon>
          <v-toolbar-title>Daftar Produk</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-col cols="3">
          ...
          ></destroy>
          <create @click.native="$router.push('/daftar-brand/form')"></create>
        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:item.brand_kategori_id="{ item }">
              {{
              item.brand_kategori ? item.brand_kategori.nama : ''
              }}
            </template>

            <template v-slot:item.cover_image="{ item }">
              <table-image v-model="item.cover_image" :type="item.type_img"></table-image>
            </template>
            ...
            <template v-slot:item.action="{ item }">
              <action
                :item="item"
                hide
                :edit="{ status: true, link: `/daftar-brand/form/` + item.id }"
                destroy
                :child="['brandBlockCount','brandFolderCount','brandImageCount', 'brandKategoriCount', 'brandKategoriProdukCount', 'brandKontakCount', 'brandProdukCount', 'brandSliderCount', 'vacancyCount']"
              >
                <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn icon v-on="on" @click="toBrand(item._id)">
                      <v-icon>mdi-arrow-right-bold-circle-outline</v-icon>
                    </v-btn>
                  </template>
                  <span>Pergi Ke Panel Produk</span>
                </v-tooltip>
              </action>
            </template>
          </v-data-table>
...
import datatable from '@/datatable/daftar-brand/index.json'
...
export default {
  ...
  methods: {
    ...
    toBrand(val) {
      this.$store.commit('app/setApp', val)

      this.$router.push('/brand/' + val)
      // window.open('/brand/' + val, '_blank');
    },
    dialogUpload() {
